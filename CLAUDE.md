...
```bash
# Clean build with tests (requires Java 8+)
./mvnw clean install
...
**Important:**
- Maven builds can be slow due to compilation and dependency resolution - use 5-10 min timeout for full builds
- GPG signing is enabled by default and will fail locally without keys - add `-Dgpg.skip=true`
- Use `./mvnw clean` to clean build artifacts instead of manually deleting directories
...
- Release creates git tags: `v{version}` (e.g., `v1.28.0`)

## Important Notes

### Java Version Compatibility
- **Current version (2.x)**: Requires Java 17+
- **Previous version (1.x)**: (this branch) Java 8+ (maintenance mode, EOL Dec 2026)
- CI/CD uses `cimg/openjdk:11.0` Docker image
